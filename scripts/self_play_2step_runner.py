#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Q-learning vs Q-learning with longer memory (Klein-style sequential pricing).

Asymmetric learning setup
-------------------------
- Firm 1: tabular Q-learning with 1-step memory.
          State = opponent's last price index (p2).
- Firm 2: tabular Q-learning with 2-step memory ("memory longer by one period").
          State = (opponent's last price index, opponent's previous price index)
          i.e., (p1_last, p1_prev).

All other ingredients follow the baseline runner:
...
Quick run example:
  python self_play_2step_runner.py --T 10000 --n_seeds 20 --seed_start 1 --csv quick_test.csv

Output CSV includes per-firm late-phase averages (avg_price_f1/f2, avg_profit_f1/f2).
"""
...
def self_play_one(
    ...
    # Exploration schedule (same as baseline)
    theta = 1.0 - (1e-6) ** (1.0 / T)

    # Firm 1: state = p2_last (n states), action = own price (n actions)
    Q1 = np.zeros((n, n))
    # Firm 2: state = (p1_last, p1_prev) (n*n states), action = own price (n actions)
    Q2 = np.zeros((n, n, n))

    # Initial prices (indices)
    p1 = int(rng.integers(n))
    p2 = int(rng.integers(n))
    # For firm 2's 2-step memory of firm 1's prices
    p1_prev = p1

    # pending tuples
    # firm1: (state_idx=p2_last, action_idx=a1, immediate_profit)
    pend1 = None
    # firm2: (state_last=p1_last, state_prev=p1_prev, action_idx=a2, immediate_profit)
    pend2 = None
    ...
    for t in range(T):
        ...
        if t % 2 == 0:
            ...
            if pend2 is not None:
                s2_last, s2_prev, a2, imm2 = pend2

                # next-period profit for firm2 uses its own price a2 vs new firm1 price a1
                next_profit2 = profit(prices, a2, a1)

                # next state for firm2 after firm1 moves: (opp_last=a1, opp_prev=old p1)
                best_future = Q2[a1, p1].max()

                target = imm2 + delta * next_profit2 + (delta ** 2) * best_future
                Q2[s2_last, s2_prev, a2] = (1.0 - alpha) * Q2[s2_last, s2_prev, a2] + alpha * target

                pend2 = None

            # set pending for firm1
            pend1 = (s1, a1, imm1)

            # update firm1 price and firm1-history for firm2
            p1_old = p1
            p1 = a1
            p1_prev = p1_old
            ...
            s2_last = p1
            s2_prev = p1_prev
            a2 = int(rng.integers(n)) if rng.random() < eps else choose(Q2[s2_last, s2_prev])
            ...
            if pend1 is not None:
                ...
                # next-period profit for firm1 uses its own price a1 vs new firm2 price a2
                next_profit1 = profit(prices, a1, a2)

                # next state for firm1 after firm2 moves is opponent_last=a2
            ...
            # set pending for firm2
            pend2 = (s2_last, s2_prev, a2, imm2)
    ...
    # Per-firm late-phase averages (to compare which learner performs better)
    ...
    # Overall average profit (for backward compatibility)
...
def main():
    ...
    ap.add_argument("--csv", type=str, default="results_2step.csv")
    args = ap.parse_args()

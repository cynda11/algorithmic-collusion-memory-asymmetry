#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Self-play Q-learning under sequential pricing (Klein-style, tabular).

What this script does
---------------------
Runs two tabular Q-learning agents that alternately set prices on a discrete grid (sequential pricing).
It repeats the simulation for many random seeds and summarizes *late-phase* behavior (last 10% of periods).

High-level setup
----------------
- Price grid: {0, 1/k, 2/k, ..., 1}.
- Alternating moves: firms take turns updating their price each period.
- Learning: epsilon-greedy exploration with a decaying schedule; tabular Q-values Q(a|s).
- Update (Klein-style two-step lookahead):
  Q(s,a) <- (1-α)Q(s,a) + α[ π_t + δ·π_{t+1} + δ^2·max_{a'} Q(s',a') ].

Timing note (important)
-----------------------
In the δ·π_{t+1} term, the agent's *own* price is the action chosen at time t
(because it keeps that price until it moves again). This script implements that timing.

Inputs (CLI)
------------
--T           Number of periods per run (default: 500000)
--k           Grid size parameter (default: 6 -> 7 price points)
--alpha       Learning rate α (default: 0.3)
--delta       Discount factor δ (default: 0.95)
--n_seeds     Number of independent runs (default: 1000)
--seed_start  First seed (default: 1)
--csv         Output CSV path (default: results.csv)

Outputs
-------
Writes a CSV with one row per run (seed). Metrics are computed on the last 10% of periods:
- avg_price  : average *market* price, i.e., mean_t (p1_t + p2_t)/2
- avg_profit : average profit across both firms, i.e., (mean π1 + mean π2)/2
- share_state_0.5_0.5, share_joint_consecutive_0.5
- mode_pair, share_mode_pair
- std_market, cycle_amp, jump_count_large
- regime (coarse label)

Also prints an aggregate summary to stdout.

Example
-------
python self_play_runner.py --T 500000 --k 6 --alpha 0.3 --delta 0.95 --n_seeds 1000 --seed_start 1 --csv results.csv
"""


import argparse
import numpy as np
import pandas as pd


def demand(p_i: float, p_j: float) -> float:
    if p_i < p_j:
        return 1.0 - p_i
    if p_i == p_j:
        return 0.5 * (1.0 - p_i)
    return 0.0


def profit(prices: np.ndarray, i_idx: int, j_idx: int) -> float:
    p_i = float(prices[i_idx])
    p_j = float(prices[j_idx])
    return p_i * demand(p_i, p_j)


def self_play_one(seed: int,
                  T: int,
                  k: int,
                  alpha: float,
                  delta: float,
                  tie_break: str = "random") -> dict:
    rng = np.random.default_rng(seed)

    prices = np.linspace(0.0, 1.0, k + 1)
    n = len(prices)

    theta = 1.0 - (1e-6) ** (1.0 / T)

    Q1 = np.zeros((n, n))
    Q2 = np.zeros((n, n))

    p1 = int(rng.integers(n))
    p2 = int(rng.integers(n))

    # pending tuples: (state_idx, action_idx, immediate_profit)
    pend1 = None
    pend2 = None

    hist_p1 = np.empty(T)
    hist_p2 = np.empty(T)
    hist_pi1 = np.empty(T)
    hist_pi2 = np.empty(T)

    def choose(Qrow: np.ndarray) -> int:
        if tie_break == "first":
            return int(np.argmax(Qrow))
        m = Qrow.max()
        best = np.flatnonzero(Qrow == m)
        return int(rng.choice(best))

    for t in range(T):
        eps = (1.0 - theta) ** t

        if t % 2 == 0:
            # firm 1 moves
            s = p2
            a = int(rng.integers(n)) if rng.random() < eps else choose(Q1[s])
            imm1 = profit(prices, a, p2)

            # update firm 2 from previous step (uses its last chosen action a2)
            if pend2 is not None:
                s2, a2, imm2 = pend2
                next_profit2 = profit(prices, a2, a)
                best_future = Q2[a].max()
                target = imm2 + delta * next_profit2 + (delta ** 2) * best_future
                Q2[s2, a2] = (1.0 - alpha) * Q2[s2, a2] + alpha * target
                pend2 = None

            pend1 = (s, a, imm1)
            p1 = a

            pi2_now = profit(prices, p2, p1)
            hist_p1[t] = prices[p1]
            hist_p2[t] = prices[p2]
            hist_pi1[t] = imm1
            hist_pi2[t] = pi2_now

        else:
            # firm 2 moves
            s = p1
            a = int(rng.integers(n)) if rng.random() < eps else choose(Q2[s])
            imm2 = profit(prices, a, p1)

            # update firm 1 from previous step (uses its last chosen action a1)
            if pend1 is not None:
                s1, a1, imm1 = pend1
                next_profit1 = profit(prices, a1, a)
                best_future = Q1[a].max()
                target = imm1 + delta * next_profit1 + (delta ** 2) * best_future
                Q1[s1, a1] = (1.0 - alpha) * Q1[s1, a1] + alpha * target
                pend1 = None

            pend2 = (s, a, imm2)
            p2 = a

            pi1_now = profit(prices, p1, p2)
            hist_p1[t] = prices[p1]
            hist_p2[t] = prices[p2]
            hist_pi1[t] = pi1_now
            hist_pi2[t] = imm2

    # late phase
    tail = int(0.1 * T)
    p1_last = hist_p1[-tail:]
    p2_last = hist_p2[-tail:]
    t_last = np.arange(T - tail, T)

    market_last = 0.5 * (p1_last + p2_last)

    mono_price = float(prices[int(np.argmin(np.abs(prices - 0.5)))])

    share_state_05_05 = float(np.mean((p1_last == mono_price) & (p2_last == mono_price)))

    mask_f1 = (t_last % 2 == 0)
    idx_even = np.where(mask_f1)[0]
    idx_even = idx_even[idx_even + 1 < len(t_last)]
    share_joint_consecutive_05 = float(np.mean((p1_last[idx_even] == mono_price) & (p2_last[idx_even + 1] == mono_price))) if len(idx_even) else 0.0

    pair_series = pd.Series(list(zip(np.round(p1_last, 6), np.round(p2_last, 6))))
    mode_pair = pair_series.value_counts().idxmax()
    share_mode_pair = float(pair_series.value_counts(normalize=True).max())

    tick = 1.0 / k
    diffs = np.diff(market_last)
    jump_count_large = int(np.sum(np.abs(diffs) > tick + 1e-12)) if len(diffs) else 0
    std_market = float(np.std(market_last))
    cycle_amp = float(np.max(market_last) - np.min(market_last))

    share_equal = float(np.mean(p1_last == p2_last))
    if share_state_05_05 > 0.8 and std_market < 0.02:
        regime = "monopoly_regime"
    elif share_equal > 0.8 and std_market < 0.02:
        regime = "focal_equal_price"
    elif cycle_amp > 0.15:
        regime = "cycling/asymmetric"
    else:
        regime = "mixed"

    avg_price = float(np.mean(market_last))
    avg_profit = float(0.5 * (np.mean(hist_pi1[-tail:]) + np.mean(hist_pi2[-tail:])))

    return {
        "seed": seed,
        "T": T,
        "k": k,
        "avg_price": avg_price,
        "avg_profit": avg_profit,
        "share_state_0.5_0.5": share_state_05_05,
        "share_joint_consecutive_0.5": share_joint_consecutive_05,
        "mode_pair": str(mode_pair),
        "share_mode_pair": share_mode_pair,
        "std_market": std_market,
        "cycle_amp": cycle_amp,
        "jump_count_large": jump_count_large,
        "regime": regime,
    }


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--T", type=int, default=500_000)
    ap.add_argument("--k", type=int, default=6)
    ap.add_argument("--alpha", type=float, default=0.3)
    ap.add_argument("--delta", type=float, default=0.95)
    ap.add_argument("--n_seeds", type=int, default=1000)
    ap.add_argument("--seed_start", type=int, default=1)
    ap.add_argument("--csv", type=str, default="results.csv")
    args = ap.parse_args()

    seeds = range(args.seed_start, args.seed_start + args.n_seeds)

    rows = []
    for i, s in enumerate(seeds, start=1):
        rows.append(self_play_one(s, args.T, args.k, args.alpha, args.delta))
        if i % max(1, args.n_seeds // 10) == 0:
            print(f"progress {i}/{args.n_seeds}")

    df = pd.DataFrame(rows)
    df.to_csv(args.csv, index=False)

    print("\n=== AGGREGATE SUMMARY (late phase) ===")
    print(f"runs: {len(df)} | T={args.T} | k={args.k} | alpha={args.alpha} | delta={args.delta}")
    print(f"avg(avg_price): {df['avg_price'].mean():.4f} | median: {df['avg_price'].median():.4f}")
    print(f"avg(avg_profit): {df['avg_profit'].mean():.4f} | median: {df['avg_profit'].median():.4f}")
    print(f"share monopoly_regime: {(df['regime']=='monopoly_regime').mean():.2%}")
    print(f"mean share_state_0.5_0.5: {df['share_state_0.5_0.5'].mean():.2%}")

    top = df.sort_values('share_state_0.5_0.5', ascending=False).head(10)
    print("\nTop 10 by share_state_0.5_0.5:")
    print(top[['seed','avg_price','avg_profit','share_state_0.5_0.5','regime','mode_pair','share_mode_pair']].to_string(index=False))

    print(f"\nSaved per-run summary to: {args.csv}")


if __name__ == "__main__":
    main()

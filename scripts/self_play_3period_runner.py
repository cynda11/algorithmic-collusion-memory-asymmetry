#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Q-learning (1-step memory) vs Q-learning with 3-period *market* memory (Klein-style sequential pricing).

Asymmetric learning setup
------------------------
- Firm 1: tabular Q-learning with 1-step memory.
  State = opponent's last price index (p2).
- Firm 2: tabular Q-learning with 3-period *overall market* memory.

We interpret "3-period" as the last three chosen prices in the alternating sequence.
When Firm 2 acts (odd t), those are: (Firm1 current price p1, Firm2 current price p2, Firm1 previous price p1_prev1).
So Firm-2 state is (p1_last, p2_last, p1_prev1).
...
def self_play_one(seed: int, T: int, k: int, alpha: float, delta: float, tie_break: str = "random") -> dict:
    ...
    Q1 = np.zeros((n, n))
    Q2 = np.zeros((n, n, n, n))  # Q2[p1, p2, p1_prev1, a2]
    ...
    pend2 = None  # (s_p1, s_p2, s_p1prev1, a2, imm2)
    ...
    for t in range(T):
        ...
        if t % 2 == 0:
            ...
            if pend2 is not None:
                s_p1, s_p2, s_p1prev1, a2, imm2 = pend2
                next_profit2 = profit(prices, a2, a1)
                # next state for firm2 becomes (p1=a1, p2=a2, p1_prev1=p1_old)
                best_future = Q2[a1, a2, p1_old].max()
                target = imm2 + delta * next_profit2 + (delta ** 2) * best_future
                Q2[s_p1, s_p2, s_p1prev1, a2] = (1.0 - alpha) * Q2[s_p1, s_p2, s_p1prev1, a2] + alpha * target
                pend2 = None
            ...
            # ----- Firm 2 moves -----
            s_p1, s_p2, s_p1prev1 = p1, p2, p1_prev1
            a2 = int(rng.integers(n)) if rng.random() < eps else choose(Q2[s_p1, s_p2, s_p1prev1])
            ...
            pend2 = (s_p1, s_p2, s_p1prev1, a2, imm2)
            p2 = a2
...
def main():
    ...
    ap.add_argument("--csv", type=str, default="results_3period.csv")
    args = ap.parse_args()
